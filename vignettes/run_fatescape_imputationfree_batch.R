...
METHOD_OUTPUT_ROOT <- "FateScape/results/Sensitivity_analysis_imputationfree"
...
# If TRUE, do not rerun cases with an existing FateScape_ImputationFree_metrics.csv unless FORCE_RERUN=TRUE.
SKIP_FINISHED <- TRUE
FORCE_RERUN <- FALSE
STOP_ON_ERROR <- FALSE

# Reconstruction parameters.
# Imputation-free variant: use barcode_observed.csv directly and do not call dropout_imputation_alter().
USE_PREIMPUTED_BARCODE <- FALSE
DISABLE_BARCODE_IMPUTATION <- TRUE
...
SAVE_INTERMEDIATE_OBJECTS <- TRUE
SAVE_FAILED_INTERMEDIATE_OBJECTS <- TRUE
GLOBAL_SEED <- 20260517
...
require_or_stop <- function(pkg) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    stop(sprintf("Required package not installed: %s\nInstall with install.packages('%s')", pkg, pkg), call. = FALSE)
  }
}

require_or_stop("ape")
require_or_stop("phangorn")
require_or_stop("TreeDist")
...
# Load FateScape. Prefer local patched source if FATESCAPE_LOCAL_PATH is available.
if (!is.null(FATESCAPE_LOCAL_PATH) && dir.exists(FATESCAPE_LOCAL_PATH)) {
  ...
  if (!requireNamespace("devtools", quietly = TRUE)) {
    stop("devtools is required for loading local FateScape source. Install with install.packages('devtools').")
  }
  devtools::load_all(FATESCAPE_LOCAL_PATH, quiet = TRUE)

  # Explicitly source patched core files after load_all, to avoid stale installed namespace.
  patched_files <- file.path(
    FATESCAPE_LOCAL_PATH,
    "R",
    c(
      "utils.R",
  ...
  for (ff in patched_files) {
    if (file.exists(ff)) {
      source(ff, local = .GlobalEnv)
      message("Sourced patched core file: ", ff)
    } else {
      warning("Patched core file not found: ", ff)
    }
  }
} else {
  library(FateScape)
  message("Loaded installed FateScape from: ", find.package("FateScape"))
}

set.seed(GLOBAL_SEED)

# Basic missing-safe sanity check.
if (exists("improved_hamming_distance", mode = "function")) {
  h1 <- improved_hamming_distance(c(NA, 1, 0), c(1, 1, 0), alpha = 0, beta = 1)
  h2 <- improved_hamming_distance(c(NA, 1, 0), c(NA, 1, 0), alpha = 0, beta = 1)
  message("Missing-safe hamming test: ", h1, ", ", h2)
}
...
standardize_barcode <- function(x) {
  # Preserve missing entries as NA for imputation-free mode.
  # Do NOT convert NA/dropout to 0, because 0 = wild type and NA = missing/dropout.
  x <- as.data.frame(x, check.names = FALSE)
  x[] <- lapply(x, function(v) {
    v <- as.character(v)
    v[v %in% c("", "NA", "NaN", "nan", "NULL", "null", "?", "-", "-1")] <- NA
...
save_reconstructed_tree_first <- function(tree, file) {
  dir.create(dirname(file), recursive = TRUE, showWarnings = FALSE)
  tree_to_save <- sanitize_phylo_for_eval(tree, "reconstructed_tree")
  ape::write.tree(tree_to_save, file = file)
...
compute_tree_metrics <- function(true_tree, pred_tree) {
  ...
  # Deterministic binary resolution stabilizes RF/TreeDist metrics for non-binary merged trees.
  ...
  rf_raw <- tryCatch(
    as.numeric(phangorn::RF.dist(true_tree_bin, pred_tree_bin, normalize = FALSE)),
    error = function(e) NA_real_
  )
  rf_norm <- tryCatch(
    as.numeric(phangorn::RF.dist(true_tree_bin, pred_tree_bin, normalize = TRUE)),
    error = function(e) NA_real_
  )
  nye <- tryCatch(
    as.numeric(TreeDist::NyeSimilarity(true_tree_bin, pred_tree_bin, normalize = TRUE)),
    error = function(e) NA_real_
  )
  td <- tryCatch(
    as.numeric(TreeDist::TreeDistance(true_tree_bin, pred_tree_bin)),
    error = function(e) NA_real_
  )
...
run_fatescape_one_case <- function(case_dir, out_dir) {
  ...
  message("\n============================================================")
  message("[FateScape-ImputationFree] Running case: ", case_id)
  ...
  # Imputation-free variant: use observed barcode directly.
  # This is the only intended difference from standard FateScape.
  if (isTRUE(DISABLE_BARCODE_IMPUTATION)) {
    barcode_for_reconstruction <- inputs$barcode_observed
    barcode_source <- "observed_no_imputation"
  } else if (USE_PREIMPUTED_BARCODE && !is.null(inputs$barcode_imputed)) {
  ...
  # Core FateScape algorithm starts here. Do not change these calls unless modifying the algorithm.
  ...
  # Core FateScape algorithm ends here.

  runtime <- proc.time() - start_time
  runtime_sec <- as.numeric(runtime[["elapsed"]])

  # Save reconstructed tree immediately before evaluation.
  tree_file <- file.path(out_dir, "FateScape_ImputationFree_reconstructed_tree.nwk")
  tree_file <- save_reconstructed_tree_first(Tree_Merge, tree_file)
  ...
  metrics$method <- "FateScape-ImputationFree"
  ...
  safe_write_csv(metrics, file.path(out_dir, "FateScape_ImputationFree_metrics.csv"), row.names = FALSE)
  ...
  run_params <- data.frame(
    ...
    disable_barcode_imputation = DISABLE_BARCODE_IMPUTATION,
    use_preimputed_barcode = USE_PREIMPUTED_BARCODE,
    ncells = ncells,
    N_char = N_char,
  ...
  safe_write_csv(run_params, file.path(out_dir, "FateScape_ImputationFree_run_params.csv"), row.names = FALSE)
  ...
  if (SAVE_INTERMEDIATE_OBJECTS) {
    saveRDS(
      ...
      file = file.path(out_dir, "FateScape_ImputationFree_intermediate_objects.rds")
    )
  }

  list(status = "success", metrics = metrics, runtime_sec = runtime_sec)
}
...
safe_write_csv(batch_plan, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_plan.csv"), row.names = FALSE)

message("Batch plan written to: ", file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_plan.csv"))
...
for (i in seq_len(nrow(batch_plan))) {
  ...
  metrics_file <- file.path(out_dir, "FateScape_ImputationFree_metrics.csv")
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      ...
      if (SAVE_FAILED_INTERMEDIATE_OBJECTS) {
        safe_dir_create(out_dir)
        saveRDS(list(error = msg, case_dir = case_dir), file = file.path(out_dir, "FateScape_ImputationFree_failed_case.rds"))
      }
  ...
  if (identical(result$status, "success")) {
    ...
    batch_records[[length(batch_records) + 1L]] <- error_records[[length(error_records)]]
  }

  # Incremental writes so that partial runs are preserved if interrupted.
  batch_manifest_tmp <- do.call(rbind, batch_records)
  safe_write_csv(batch_manifest_tmp, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_manifest.csv"), row.names = FALSE)

  if (length(metrics_records) > 0) {
    metrics_all_tmp <- do.call(rbind, metrics_records)
    safe_write_csv(metrics_all_tmp, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_metrics_all.csv"), row.names = FALSE)
  }

  if (length(error_records) > 0) {
    errors_tmp <- do.call(rbind, error_records)
    safe_write_csv(errors_tmp, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_errors.csv"), row.names = FALSE)
  }
}

batch_manifest <- do.call(rbind, batch_records)
safe_write_csv(batch_manifest, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_manifest.csv"), row.names = FALSE)

if (length(metrics_records) > 0) {
  metrics_all <- do.call(rbind, metrics_records)
  safe_write_csv(metrics_all, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_metrics_all.csv"), row.names = FALSE)
  message("Batch metrics written to: ", file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_metrics_all.csv"))
}

if (length(error_records) > 0) {
  errors_all <- do.call(rbind, error_records)
  safe_write_csv(errors_all, file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_errors.csv"), row.names = FALSE)
  message("Batch errors written to: ", file.path(SUMMARY_DIR, "FateScape_ImputationFree_batch_errors.csv"))
}

message("\nBatch completed.")
if (nrow(batch_manifest) > 0) print(table(batch_manifest$status))
